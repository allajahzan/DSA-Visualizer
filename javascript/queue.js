...
function findPeek(event){
    ...
    let intervel = setInterval(() => {
        ...
        if (box.style.backgroundColor === 'rgb(247, 128, 43)') {
            document.getElementById(`${0}`).style.backgroundColor = 'chocolate'
...
function insertAtLast(event) {
    ...
    if (!localStorage.getItem('gaveExplanation')) {
        ...
        note_pad.innerHTML = `
<p style="font-size:16px;">Queue has two end. One is front another one is rare. So when we add a value to queue that will takes place at the rare. And when we remove a value from the queue that will takes place at the front.</p>
...
function pushValue(){
    ...
    let intervel = setInterval(() => {
        ...
        if (box.style.backgroundColor === 'rgb(247, 128, 43)'){
            ...
            document.getElementById(`stack${array.length - 1}`).style.backgroundColor = 'chocolate'
            document.getElementById('in').style.color = 'green'
        }
        else{
            document.getElementById(`${array.length - 1}`).style.backgroundColor = 'rgb(247, 128, 43)'
            document.getElementById(`stack${array.length - 1}`).style.backgroundColor = 'rgb(247, 128, 43)'
            document.getElementById('in').style.color = 'black'
        }
    }, 100);
    ...
    setTimeout(() => {
        ...
        document.getElementById(`stack${array.length - 1}`).innerHTML = val
        clearInterval(intervel);

        document.getElementById(`${array.length - 1}`).style.backgroundColor = ''
        document.getElementById(`stack${array.length - 1}`).style.backgroundColor = ''
...
function removeValue(event) {
    ...
    if (!localStorage.getItem('gaveExplanation1')) {
        ...
        note_pad.innerHTML =`
<p style="font-size:16px;">Queue has two end. One is front another one is rare. So when we add a value to queue that will takes place at the rare. And when we remove a value from the queue that will takes place at the front.</p>
 <button style="padding:0px;font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; font-weight:500; width: 50px;font-size: 16px;padding-bottom: 4px;" onclick=shiftValue()>ok</button> 
`
